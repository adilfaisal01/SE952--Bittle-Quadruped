...
H = torch.rand(num_envs, dtype=torch.float32, device=device) * 5 + 3         # clearance 3–8
x_COMshift = torch.rand(num_envs, dtype=torch.float32, device=device) * 10 - 25  # -25 to -15 mm
robotheight = torch.rand(num_envs, dtype=torch.float32, device=device) * 5 + 15  # 15–20 mm
dutycycle = torch.rand(num_envs, dtype=torch.float32, device=device) * 0.5 + 0.4  # 0.4–0.9
forwardvel = torch.rand(num_envs, dtype=torch.float32, device=device) * 200 + 50  # 50–250 mm/s
T = 1 / (torch.rand(num_envs, dtype=torch.float32, device=device) * 2 + 1)  # periods 0.5–1 Hz
yaw_rate = torch.rand(num_envs, dtype=torch.float32, device=device) * 0.2 - 0.1  # -0.1–0.1 rad/s


torch_start=time.time()
# Torch vectorized gait parameters (batched)
gait_envs_torch = gaitParams(
    H=H,
...
for _ in range(num_timesteps-1):
    Q = oscillator.tensor_hopf_cpg_dot(Q, R_trot, delta=0.5, b=50, mu=1, alpha=10, gamma=10, dt=dt)
...
for env_idx, gait in enumerate(gait_envs_np):
    ...
    for t_idx in range(num_timesteps):
        ...
        if t_idx < num_timesteps-1:
            Q_np = osc.hopf_cpg_dot(Q_np, R_np, delta=0.5, b=50, mu=1, alpha=10, gamma=10, dt=dt)
...
#         plt.plot(Time, x_traj_np[env, :, leg_idx], label=f"NumPy Env {env+1}", marker='o')
#         plt.plot(Time, x_traj_torch[:, env, leg_idx], "x", label=f"Torch Env {env+1}")
#     plt.title(f"{leg_name} X Trajectory")
#     plt.xlabel("Time (s)"); plt.ylabel("X (mm)"); plt.grid(True); plt.legend(); plt.show()
#     plt.pause(0.5)

#     # --- Z Trajectory ---
#     plt.figure(figsize=(12,5))
#     for env in range(num_envs):
#         plt.plot(Time, z_traj_np[env, :, leg_idx], label=f"NumPy Env {env+1}", marker='o')
#         plt.plot(Time, z_traj_torch[:, env, leg_idx], "x", label=f"Torch Env {env+1}")
#     plt.title(f"{leg_name} Z Trajectory")
#     plt.xlabel("Time (s)"); plt.ylabel("Z (mm)"); plt.grid(True); plt.legend(); plt.show()
#     plt.pause(0.5)

#     # --- Hip Angles ---
#     plt.figure(figsize=(12,5))
#     for env in range(num_envs):
#         plt.plot(Time, hip_np[env, :, leg_idx], label=f"NumPy Env {env+1}", marker='o')
#         plt.plot(Time, hip_torch[:, env, leg_idx], "x", label=f"Torch Env {env+1}")
#     plt.title(f"{leg_name} Hip Angle")
#     plt.xlabel("Time (s)"); plt.ylabel("Angle (rad)"); plt.grid(True); plt.legend(); plt.show()
#     plt.pause(0.5)

#     # --- Knee Angles ---
#     plt.figure(figsize=(12,5))
#     for env in range(num_envs):
#         plt.plot(Time, knee_np[env, :, leg_idx], label=f"NumPy Env {env+1}", marker='o')
#         plt.plot(Time, knee_torch[:, env, leg_idx], "x", label=f"Torch Env {env+1}")
#     plt.title(f"{leg_name} Knee Angle")
#     plt.xlabel("Time (s)"); plt.ylabel("Angle (rad)"); plt.grid(True); plt.legend(); plt.show()
#     plt.pause(0.5)
